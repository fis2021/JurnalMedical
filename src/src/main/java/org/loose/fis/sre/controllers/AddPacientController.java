...
public class AddPacientController {
    ...
    public void handleAddPacientButtonAction() throws Exception{
        try{int g=MedicService.addPacient(LoginController.medic,pacientField.getText());
        if(g==0) pacientMessage.setText("Pacientul nu este inregistrat!");
        if(g==1) {if(stg!=null)stg.close();
        ...
        catch(PatientAlreadyExistsException e){pacientMessage.setText(e.getMessage());}catch (Exception e) {
            e.printStackTrace();
        }
    }
    @FXML
